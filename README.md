...
기존 WM 서비스는 미래를 단일 숫자로 표현합니다.  
LifeLong WM은 미래를 **확률 분포**로 표현합니다.

```
기존 서비스: "83세까지 버팁니다"
LifeLong WM: "83세 전 자산 고갈 확률은 41%입니다"
```
...
[사용자 — 버튼 클릭]
        ↓
[llama3.1:8b]  ──→  어떤 통계 도구를 쓸지 결정 + Tool 호출
        ↓
[통계 모델 (Role A — 순수 Python)]
  ...
  · anomaly/       — 이상 거래 탐지
        ↓
[exaone3.5:7.8b]  ──→  계산 결과를 자연스러운 한국어로 설명
        ↓
[Streamlit UI]  ──→  팬 차트 + 결과 텍스트 (연령별 글씨 크기 자동 조정)
```

**LLM은 완전 로컬 (Ollama) — API 비용 없음**
...
| 영역 | 기술 |
|------|------|
| LLM (Tool 선택) | Ollama · llama3.1:8b |
| LLM (한국어 응답) | Ollama · exaone3.5:7.8b (LG AI Research) |
...
JB_FinAI/
├── agent/
│   ├── agent.py       # 이중 모델 Agent 루프 (llama3.1 + exaone3.5)
...
버튼 기반 인터페이스 — 어르신이 LLM과 직접 대화하지 않아도 됨
